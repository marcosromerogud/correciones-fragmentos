...
- **`banners-roto/`** — a known-good vs. known-broken pair (`ejemplo.html` vs `roto.html`) used as a reference for a specific corruption pattern: Adobe Campaign's editor can strip the value from `class="is-highlight"` on `<img>` tags, leaving a bare `class` attribute. `is-highlight` must always be preserved with its value on tracked images — treat a bare `class` attribute on an `<img>` as a red flag when reviewing fragments.
...
- **`is-highlight` on images**: never drop or empty this attribute value; it's load-bearing for Adobe Campaign and is the specific failure mode documented in `banners-roto/`.
